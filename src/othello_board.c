...
int othelloBoard_count(size_t begin, size_t end, int value)
{
    int count = 0;

    for(size_t i= begin; i<= end; i++)
    {
        if(value == othello_board.positions[i])
        {
            count++;
        }
    }
 ...
 * @brief 初始化井字棋棋盘
 *
 * 该函数将井字棋棋盘初始化，将所有格子置为0，并设置棋盘上的棋子数量为4。
 */
void Init_othelloBoard() {
    memset(&othelloBoard,0,sizeof(othelloBoard))
    othello_board.discsOnBoard = 4;
 ...
 */
void othelloBoard_displayBoard(int color) {
    // 打印棋盘标题
    printf("\n");
    ...
    for (int i = 0; i < 64; i += 8) {
        ...
        for (int j = i; j < i+8; j++) {
            // 检查当前位置是否有棋子
            if (othello_board.positions[j] == 1) {
                // 打印黑色棋子
                // Black disc followed by green space
                printf("\033[48;5;34m\033[38;5;232m\u2022 \033[0m"); // 打印黑色棋子后跟绿色空格
            }
                ...
                printf("\033[48;5;34m\033[38;5;232m\u2613 \033[0m"); // 打印黑色X后跟绿色空格
            } 
            // TODO:
                ...
                printf("\033[48;5;34m\033[38;5;232m\u00B7 \033[0m"); // 打印点后跟绿色空格
            }
        }

        // 在棋盘底部打印双方棋子数量
        if (i == 24) {
            printf("\t\tBlack: %d",othelloBoard_count(0,63, OTHELLO_BOARD_BLACK)); // 打印黑色棋子数量
        }
        else if (i == 32) {
            printf("\t\tWhite: %d",othelloBoard_count(0,63, OTHELLO_BOARD_WHITE)); // 打印白色棋子数量
        }
...
void othelloBoard::findLegalMoves(int color,
    ...
    for (int i = OTHELLO_BOARD_BEGIN_INDEX; i < OTHELLO_BOARD_SIZE; i++) {
